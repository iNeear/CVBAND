...
# Función para enviar datos del puerto serie
def enviar_datos(motor_pwm, servo_comando):
    datos = (motor_pwm, servo_comando) # Crear una tupla con los datos del motor y del servo
    serialConnection.write(str(datos).encode()) # Convertir la tupla a una cadena y enviarla por el puerto serie
...
def motorControl(value):
    ...
    enviar_datos(pwmValue, None) # Envía el valor de PWM al dispositivo conectado por el puerto serie, como None en el segundo argumento indica que no hay comando de servo

# Funciones para enviar los comandos del servo motor
def enviar_comando(comando):
    enviar_datos(None, comando) # Envía el comando de servo al dispositivo conectado por el puerto serie, como None en el primer argumento indica que no hay valor de PWM del motor

def estado_1():
    enviar_comando("a") # Comando a 70°

def estado_2():
    enviar_comando("b") # Comando a 45°

def estado_3():
    enviar_comando("c") # Comando a 0°
...
btn_estado_1 = Button(root, text="Estado 1 (Rojo)", command=estado_1)
btn_estado_2 = Button(root, text="Estado 2 (Amarillo)", command=estado_2)
btn_estado_3 = Button(root, text="Estado 3 (Verde)", command=estado_3)

# Posicionar los botones en la ventana
btn_estado_1.grid(row=0, column=0, padx=5, pady=5)
btn_estado_2.grid(row=1, column=0, padx=5, pady=5)
btn_estado_3.grid(row=2, column=0, padx=5, pady=5)

# Posicionar las otras widgets en la ventana
canvas = FigureCanvasTkAgg(fig, master=root)
canvas._tkcanvas.grid(row=0, column=1, rowspan=3, padx=10, pady=10)
...
label.grid(row=1, column=2, padx=5, pady=5, sticky="n")

boton = Button(root, text="Detener", bg="gray", command=root.quit)
boton.grid(row=2, column=2, padx=5, pady=5, sticky="s")
...
# Función para controlar el motor
def motorControl(value, comando):
    pwmValue = str(value) # Convierte el valor de PWM a una cadena de caracteres
    pwm.set("PWM: " + pwmValue) # Actualiza el valor de la etiqueta de PWM
    if comando is not None: # Si el valor del comando no es nulo, crea una tupla con ambos datos
        data = (pwmValue, comando)
        serialConnection.write(str(data).encode()) # Envía la tupla al dispositivo conectado por el puerto serie
    else: # Si el valor del comando es nulo, envía solamente el valor de PWM
...
def actualizar_setpoint():
    setpoint = float(entry.get()) # Obtener el valor ingresado en el Entry widget
    s1.set(setpoint) # Actualizar el valor del control s1 con el setpoint
    motorControl(setpoint, None) # Actualizar el valor del PWM en el motor sin enviar ningún comando

# Funciones para enviar los comandos al Arduino
def enviar_comando(value, comando):
    data = (str(value), comando) # Crea una tupla con ambos valores
    serialConnection.write(str(data).encode()) # Envía la tupla al dispositivo conectado por el puerto serie

def estado_1():
    enviar_comando(value, "a")

def estado_2():
    enviar_comando(value, "b")

def estado_3():
    enviar_comando(value, "c")
